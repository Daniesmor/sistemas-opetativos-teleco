...
char **
reserve_args_token(Command *cmd, int token_size)
{
	cmd->argumentos[cmd->numArgumentos] =
	    malloc((token_size + 1) * sizeof(char));
	return cmd->argumentos;
}
	...
	for (int i = 0; i < NUM_PARAMS; i++) {
		...
		while (token != NULL) {
			...
			if (token != NULL) {
				...
				if (reserve_args_token
				    (Commands[i], strlen(token)) == NULL) {
					// Manejar el error de asignación de memoria
					memLocateFailed();
					free(argv_copy);
					return;
...
executeCommands(Command *Commands[])
{
	...
	for (int numCommand = 0; numCommand < NUM_PARAMS; numCommand++) {
		...
		case 0:
			...
			execv(Commands[numCommand]->path,
			      Commands[numCommand]->argumentos);

...
initializerForbid()
{
	forbid_cmds.numCommands = 0;
	forbid_cmds.comandos = NULL;
}
...
exec_forbid(Command *cmd)
{
	...
	if (forbid_cmds.comandos == NULL) {

		printf("%s \n",cmd->argumentos[1]);
		forbid_cmds.comandos = realloc(forbid_cmds.comandos, (forbid_cmds.numCommands +1)* sizeof(char *));
		forbid_cmds.comandos[forbid_cmds.numCommands] = malloc(strlen(cmd->argumentos[1])*sizeof(char)+1);
		
		forbid_cmds.comandos[forbid_cmds.numCommands][0] = '\0'; 
		
		strcpy(forbid_cmds.comandos[forbid_cmds.numCommands], cmd->argumentos[1]);
		...
		strcat(forbid_cmds.comandos[forbid_cmds.numCommands], cmd->argumentos[1]);;

	}

	//printf("COmando prohibido: %s \n", forbid_cmds.comandos[0]);
	forbid_cmds.numCommands = forbid_cmds.numCommands + 1;

	for (int numCommand = 0; numCommand < forbid_cmds.numCommands; numCommand++) {

		printf("COmando prohibido: %s \n", forbid_cmds.comandos[numCommand]);
...
{
	/*
	for (int numCommandForbid = 0; numCommandForbid < forbid_cmds.numCommands; numCommandForbid++) {

			if (strcmp(cmds->comandos[numCommand]->nombre,forbid_cmds.comandos[numCommandForbid])== 0) {
				forbid = forbid +1;
				printf("Command forbiden \n");
			}
